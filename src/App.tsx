...
function App() {
  ...
  const handleFileChange = (value: string | undefined) => {
    if (!value) return;
    setFiles(files.map(file => file.name === activeFile.name ? { ...file, content: value } : file));
  };
  ...
  const handleSaveRename = () => {
    ...
    setFiles(files.map(file => file.name === renameFile ? { ...file, name: newFileNameForRename } : file));
    setRenameFile(null);
  };
...
<!-- stack-rush.vercel.app -->`;
  ...
  return (
    <div className="app-container">
      <div className="theme-switcher" style={{ display: "flex", justifyContent: "space-between", alignItems: "center", padding: "8px" }}>
        <img src="/Computer.svg" alt="computer" style={{ width: "30px", marginLeft: "10px", verticalAlign: "middle" }} />
        <div style={{ display: "flex", alignItems: "center", gap: "10px" }}>
          <label style={{ fontWeight: "500" }}>stack-rush.vercel.app</label>
          <select value={theme} onChange={(e) => setTheme(e.target.value)} style={{ padding: "4px 8px", borderRadius: "4px", border: "1px solid #ccc" }}>
            <option value="vs-dark">Dark</option>
            <option value="light">Light</option>
          </select>
        </div>
      </div>
      <button onClick={() => setShowDeployModal(true)}>Deploy Button Generator</button>
      <SplitPane split="vertical" sizes={sizes} onChange={setSizes} sashRender={sashRender}>
        <div className={`files-panel ${isDragging ? "dragging" : ""}`} onDragOver={(e) => { e.preventDefault(); setIsDragging(true); }} onDragLeave={() => setIsDragging(false)} onDrop={async (e) => {
          e.preventDefault();
          setIsDragging(false);
          ...
          for (const file of droppedFiles) {
            ...
            setFiles((prev) => [...prev, { name: file.name, content: text, type }]);
          }
        }}>
          <h2>Website Builder</h2>
          <div className="drag-drop-zone" style={{ border: "2px dashed #999", padding: "1rem", marginBottom: "1rem", textAlign: "center", borderRadius: "8px", backgroundColor: isDragging ? "#f0f0f0" : "transparent", transition: "background-color 0.2s ease" }}>
            + Add Files
          </div>
          {files.map((file) => (
            <div key={file.name} className={`file-item ${file.name === activeFile.name ? "active" : ""}`} onClick={() => setActiveFile(file)}>
              {renameFile === file.name ? (
                <div className="rename-input">
                  <input type="text" value={newFileNameForRename} onChange={(e) => setNewFileNameForRename(e.target.value)} />
        ...
        <SplitPane split="vertical" sizes={[60, 40]} onChange={setSizes} sashRender={sashRender}>
          <div className="editor-panel">
            <Editor height="100%" defaultLanguage={activeFile.type} value={activeFile.content} onChange={handleFileChange} theme={theme} options={{ minimap: { enabled: false }, fontSize: 14, wordWrap: "on", padding: { top: 30 } }} />
          </div>
          <div className="preview-panel">
            <div className="button-panel">
              ...
              <img src="/react.svg" alt="React Logo" style={{ width: "30px", marginLeft: "10px", verticalAlign: "middle" }} />
              <img src="/typescript.svg" alt="TypeScript Logo" style={{ width: "30px" }} />
            </div>
            <iframe srcDoc={preview} title="preview" style={{ width: "100%", height: "100%", border: "none" }} />
          </div>
        </SplitPane>
      </SplitPane>
