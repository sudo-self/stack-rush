...
function App() {
  ...
  useEffect(() => {
    ...
    if (activeFile.type === "md") {
      combined = `<!DOCTYPE html><html><head><meta charset='UTF-8' /><script src='https://cdn.tailwindcss.com'></script><style>body { font-family: sans-serif; padding: 20px; } h1, h2, h3 { font-weight: bold; margin-top: 1em; } pre { background: #f3f4f6; padding: 10px; border-radius: 6px; } code { background: #e5e7eb; padding: 2px 4px; border-radius: 4px; }</style></head><body>${marked(mdContent)}</body></html>`;
    } else {
      combined = `<!DOCTYPE html><html><head><meta charset='UTF-8' /><script src='https://cdn.tailwindcss.com'></script><style>${cssContent}</style></head><body>${htmlContent}<script>${jsContent}</script></body></html>`;
    }
      ...
      <SplitPane split="vertical" sizes={sizes} onChange={setSizes} sashRender={sashRender}>
        ...
        <SplitPane split="vertical" sizes={[60, 40]} onChange={setSizes} sashRender={sashRender}>
          <Editor
            ...
            theme="vs-dark"
            options={{ minimap: { enabled: false }, fontSize: 14, wordWrap: "on", padding: { top: 30 } }}
          />
          <div>
      ...
      {showEmbedModal && (
        <div className="modal-overlay" style={{ position: "fixed", top: 0, left: 0, right: 0, bottom: 0, backgroundColor: "rgba(0,0,0,0.5)", display: "flex", justifyContent: "center", alignItems: "center", zIndex: 9999 }}>
          <div style={{ backgroundColor: "white", padding: "1rem", borderRadius: "6px", width: "90%", maxWidth: "600px" }}>
            <h2>Embed HTML</h2>
            <textarea readOnly value={embedCode} style={{ width: "100%", height: "150px", padding: "8px", fontFamily: "monospace", borderRadius: "4px" }} />
            <div style={{ marginTop: "8px", display: "flex", justifyContent: "flex-end", gap: "8px" }}>
              <button onClick={() => setShowEmbedModal(false)}>Close</button>
              <button onClick={() => navigator.clipboard.writeText(embedCode).then(() => alert("Embed code copied!"))}>Copy</button>
            </div>
          </div>
        </div>
      )}
